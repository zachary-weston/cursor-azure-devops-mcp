 ...
 */
export function safeResponse(result) {
            ...
            if (parsedContent && parsedContent._readableState && parsedContent._readableState.buffer) {
                ...
                if (Array.isArray(bufferData) &&
                    ...
                    try {
                        ...
                        const isLikelyText = !result.isBinary &&
                            (!result.contentType ||
...
server.tool('azure_devops_projects', 'List all Azure DevOps projects in your organization. Returns project details including ID, name, description, visibility, and state. Results are automatically truncated if they exceed size limits.', {}, async () => {
    try {
...
server.tool('azure_devops_work_item', 'Get detailed information about a specific work item by ID. Returns work item fields, relations, history, and metadata. Supports all work item types (bugs, tasks, user stories, etc).', {
...
server.tool('azure_devops_work_items', 'Get detailed information for multiple work items by their IDs. Returns complete work item data including fields, relations, history, and metadata for each item. Useful for bulk retrieval of work items. Response is automatically truncated if it exceeds size limits while preserving essential data.', {
    ids: z
        .array(z.number())
        .describe('Array of work item IDs to retrieve. Each ID should be a valid work item identifier.'),
}, async ({ ids }) => {
    try {
...
server.tool('azure_devops_pull_requests', 'List all pull requests in a repository. Returns comprehensive PR information including title, description, status, reviewers, work items, commits, and thread counts. Supports filtering and pagination. Results are automatically truncated if they exceed size limits.', {
    repositoryId: z
        .string()
    ...
    project: z
        .string()
        .optional()
        .describe('Project name containing the repository. If not provided, uses the default project from configuration.'),
...
server.tool('azure_devops_pull_request_by_id', 'Get detailed information about a specific pull request. Returns complete PR data including title, description, status, reviewers, work items, commits, policy evaluations, and merge status. Useful for getting the full context of a PR. Response includes all associated metadata.', {
    repositoryId: z
        .string()
        .describe('Repository ID containing the pull request. Must be a valid repository identifier.'),
    pullRequestId: z
        .number()
...
server.tool('azure_devops_pull_request_threads', 'Get all comment threads in a pull request. Returns thread details including comments, status, relationships, and file annotations. Supports both file-specific and general comments. Results include thread context and metadata.', {
    repositoryId: z
        .string()
        .describe('Repository ID containing the pull request. Must be a valid repository identifier.'),
    pullRequestId: z
...
server.tool('azure_devops_work_item_attachments', 'Get all attachments associated with a work item. Returns attachment details including file name, size, creator, timestamps, and download URLs. Supports all attachment types including images, documents, and binary files. Results include metadata for each attachment.', {
    id: z
        .number()
...
server.tool('azure_devops_work_item_links', 'Get all links and relationships for a work item. Returns detailed link information including link types, target work items, and relationship attributes. Supports all link types (Parent/Child, Related, Predecessor/Successor, etc). Results include full context of work item relationships.', {
    id: z
        .number()
...
server.tool('azure_devops_pull_request_file_content', 'Get the content of a specific file in a pull request. By default returns the complete file as plain text. Set returnPlainText=false to get content in chunks with metadata. Has a 5-minute timeout for large files.', {
    repositoryId: z.string().describe('Repository ID'),
    pullRequestId: z.number().describe('Pull request ID'),
    ...
    returnPlainText: z
        .boolean()
        .optional()
        .describe('When true (default), returns complete file as plain text; when false, returns JSON with chunk details')
        .default(true),
}, async ({ repositoryId, pullRequestId, filePath, objectId, startPosition, length, project, returnPlainText, }, _context) => {
    try {
        if (returnPlainText) {
            // Get the complete file content
            const content = await azureDevOpsService.getCompletePullRequestFileContent(repositoryId, pullRequestId, filePath, objectId, project);
...
server.tool('azure_devops_branch_file_content', 'Get the content of a file directly from a branch. By default returns the complete file as plain text. Set returnPlainText=false to get content in chunks with metadata. Has a 5-minute timeout for large files.', {
    repositoryId: z.string().describe('Repository ID'),
    ...
    returnPlainText: z
        ...
        .describe('When true (default), returns complete file as plain text; when false, returns JSON with chunk details')
        .default(true),
}, async ({ repositoryId, branchName, filePath, startPosition, length, project, returnPlainText }, _context) => {
    try {
        if (returnPlainText) {
            // Get the complete file content
            const content = await azureDevOpsService.getCompleteFileFromBranch(repositoryId, filePath, branchName, project);
            // Return in the proper MCP format
            return {
                content: [
                    {
            ...
            // Get the file content in chunks with metadata
            const result = await azureDevOpsService.getFileFromBranch(repositoryId, filePath, branchName, startPosition, length, project);
            // Format the response properly
            return {
                content: [
                    {
...
server.tool('azure_devops_create_pr_comment', 'Create a comment on a pull request', {
    repositoryId: z.string().describe('Repository ID'),
    pullRequestId: z.number().describe('Pull request ID'),
    project: z.string().describe('Project name'),
...
// Test Plans
server.tool('azure_devops_test_plans', 'List all test plans in a project. Returns test plan details including ID, name, area path, and iteration path. Results are automatically truncated if they exceed size limits while preserving essential metadata.', {
    project: z
        .string()
        .optional()
        .describe('The name of the project. If not provided, uses the default project from configuration.'),
}, async ({ project }) => {
    try {
...
server.tool('azure_devops_test_plan', 'Get a test plan by ID', {
    project: z.string().describe('Project name'),
...
// Test Suites
server.tool('azure_devops_test_suites', 'List all test suites for a test plan', {
    project: z.string().describe('Project name'),
    testPlanId: z.number().describe('Test plan ID'),
}, async ({ project, testPlanId }) => {
    try {
...
server.tool('azure_devops_test_suite', 'Get detailed information about a specific test suite by ID. Returns suite details, test cases, configurations, and related information. Supports all test suite types (static, requirement-based, query-based).', {
    project: z
        .string()
        .optional()
        .describe('The name of the project. If not provided, uses the default project from configuration.'),
...
// Test Cases
server.tool('azure_devops_test_cases', 'List all test cases for a test suite', {
    project: z.string().describe('Project name'),
    testPlanId: z.number().describe('Test plan ID'),
...
server.tool('azure_devops_wiql_query', 'Execute a Work Item Query Language (WIQL) query and return the results. Supports all query operators and clauses including WHERE, ORDER BY, etc. Returns work item references that can be retrieved using azure_devops_work_items.', {
    query: z.string().describe('The WIQL query to execute. Must be a valid WIQL query string.'),
    project: z
        .string()
        .optional()
        .describe('Project name for project-scoped queries. If not provided, uses the default project from configuration.'),
...
server.tool('azure_devops_update_work_item', 'Update a work item with new field values, relations, or comments. Supports all work item types (bugs, tasks, user stories, etc). Returns the updated work item.', {
...
server.tool('azure_devops_create_test_case', 'Create a new test case in a test suite. Returns the created test case details including ID, name, and URL.', {
...
async function main() {
    ...
    try {
        ...
        console.error('- azure_devops_branch_file_content: Get content of a file directly from a branch');
